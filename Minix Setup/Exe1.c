...
int do_newexec()
{
    ...
    rmp = &mproc[proc_n];
    ptr = m_in.EXC_NM_PTR;
    r = sys_datacopy(who_e, (vir_bytes)ptr, SELF, (vir_bytes)&args, sizeof(args));
    if (r != OK)
        panic("do_newexec: sys_datacopy failed: %d", r);
    allow_setuid = 0;
    /* Do not allow setuid execution */
    rmp->mp_flags &= ~TAINTED;
    /* By default not tainted */
    if (rmp->mp_tracer == NO_TRACER)
    {
        /* Okay, setuid execution is allowed */ allow_setuid = 1;
    }
    ...
    /* A process is considered 'tainted' when it's executing with * setuid or setgid bit set, or when the real{u,g}id doesn't * match the eff{u,g}id, respectively. */
    if (allow_setuid && args.allow_setuid)
    {
    ...
    printf("Executing %s", &rmp->mp_name);
...
void exec_restart(rmp, result, pc, vfs_newsp)
{
    ...
    rmp->mp_flags &= ~PARTIAL_EXEC; /* Fix 'mproc' fields, tell kernel that exec is done, reset caught * sigs. */
    ...
    } /* Cause a signal if this process is traced. * Do this before making the process runnable again! */
#if USE_TRACE if (rmp->mp_tracer != NO_TRACER && !(rmp->mp_trace_flags & TO_NOEXEC))
    {
        sn = (rmp->mp_trace_flags & TO_ALTEXEC) ? SIGSTOP : SIGTRAP;
        check_sig(rmp->mp_pid, sn, FALSE /* ksig */);
    }
#endif /* USE_TRACE */ /* Call kernel to exec with SP and PC set by VFS. */ r = sys_exec(rmp->mp_endpoint, (char *)vfs_newsp, rmp->mp_name, pc);
    if (r != OK)
